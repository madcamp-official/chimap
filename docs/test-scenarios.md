...
1. 첫 방문 인트로와 건너뛰기
2. 최초 방문이면 출생연도·신장·체중·생물학적 성별 필수 입력
3. 개인화 한 걸음 길이와 브라우저 전용 저장 안내 확인
4. `대전 유성구 대학로 291` 검색
5. 한국과학기술원 직접 선택
6. 대전역 검색·직접 선택
7. 걸음·목표·마감·추가시간 입력
...
17. localStorage v2에 필수 개인화 프로필과 이동 설정 저장
18. 새로고침 후 입력 복구
...
2. 20m 보행 측정 입력·저장 필드가 존재하지 않음
3. 동일 입력에서 모든 거리↔걸음 계산이 `stepLengthMeters`를 사용
4. 마지막 버스에 조기 하차 가능 정류장이 있으면 해당 후보를 먼저 조회
5. 조기 하차 후보가 목표 ±5%이면 늦은 탑승 API 호출 생략
6. 부족한 경우에만 늦은 탑승과 양쪽 조합을 순서대로 보완
7. 환승 지점 유지, 버스 leg 최소 한 정거장, 승하차 순서 보장
