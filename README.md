...
CHIMap은 목적지와 도착 마감시간을 유지하면서 더 많이 걸을 수 있는 실제
...
2. 캠퍼스 중심·도로명 주소·출입구 표시를 확인하고 검색 결과를 직접
   선택합니다.
3. 최초 이용 시 출생연도·신장·체중·생물학적 성별로 개인화 한 걸음
   길이를 계산합니다. 네 항목은 모두 필수이며 원본 신체정보는 브라우저에만
   저장합니다. 직접 한 걸음 길이를 입력하거나 일정 거리를 걷게 하는 측정
   절차는 사용하지 않습니다.
4. 현재 걸음, 목표 걸음, 마감시간, 추가 허용시간을 입력합니다.
5. TAGO 버스, Kakao 도보와 도로 매칭 geometry를 조합한 경로를
   계산합니다.
