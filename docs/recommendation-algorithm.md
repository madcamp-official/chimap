...
선택 경로의 상세 geometry가 도보 시간과 거리를 바꾸면 최종 추가시간은 같은
상세화 단계의 FAST 시간을 기준으로 다시 계산합니다. 상세화 전 FAST와 상세화
후 GOAL을 섞어 비교하지 않으며, 최대 추가시간과 도착 마감 정책은 그대로
적용합니다. 응답의 `baseline`도 최종 상세 FAST의 시간·도착시각·도보거리·
