...
2. TAGO ID가 연결된 가까운 정류장을 각각 최대 8개 사용합니다.
3. 정류장별 노선과 노선 정류장 순서를 DB 우선으로 조회합니다.
4. 양쪽에 공통인 노선에서 승차 순서<하차 순서인 직행 후보를 찾습니다.
5. 직행 실제 후보를 최대 3개 구성합니다.
6. 직행 결과가 2개 미만이고 설정이 허용하면 최대 1회 환승을 탐색합니다.
7. 가장 짧은 총 소요시간 후보를 baseline으로 사용합니다.
...
기본값은 평균 버스 속도 20km/h, 정차시간 25초입니다. 버스 polyline은
실제 도로 shape가 아니라 TAGO 노선 정류장 좌표를 순서대로 연결한 선이며,
응답의 `estimationNotes`에 이를 명시합니다.
